#!/usr/bin/env bash
# rapid-team.sh — launch the RAPID build TEAM as terminal agents.
#
...
# per-dimension review subagents). Run from a project root (needs .rapid/STATE.json).
#
#   rapid-team.sh                 launch the team via tmux (session: rapid-team)
#   rapid-team.sh --dry-run       print what would launch; do nothing
#   rapid-team.sh --cursor        write .vscode/tasks.json so each role opens in
#                                 its own Cursor / VS Code integrated terminal
#   rapid-team.sh --track fast    smaller team (planner, coder, reviewer, watchdog)
#   rapid-team.sh /path/to/proj   target a specific project root
#
# Context + hooks: each window exports RAPID_ROLE=<role> and reads its contract
# at 04-spec/agents/<role>.md (seeded from templates/agent-roles/). The globally
# installed RAPID hooks (phase-gate R1, continuation R7, conformance R8, stub R9,
# status/heartbeat) attribute themselves per RAPID_ROLE automatically — no
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    -*)        echo "rapid-team: unknown flag $1" >&2; exit 2 ;;
    *)         PROJECT="$1" ;;
  esac; shift
done
PROJECT="$(cd "$PROJECT" 2>/dev/null && pwd)" || { echo "rapid-team: bad project path" >&2; exit 2; }
TOOLS_DIR="$(cd "$(dirname "$0")" && pwd)"
TPL_DIR="$(cd "$TOOLS_DIR/../templates/agent-roles" 2>/dev/null && pwd || true)"

[ -f "$PROJECT/.rapid/STATE.json" ] || { echo "rapid-team: not a RAPID build (no .rapid/STATE.json in $PROJECT)" >&2; exit 2; }
...
mission() { case "$1" in
  ...
  coder)    echo "build lead — implement tasks from .rapid/TASKS.json, fanning out coding subagents for independent tasks; keep-or-revert on the tester's results; never self-approve" ;;
  ...
  watchdog) echo "R2 auditor — independently audit completed work against spec/architecture for drift; write .rapid/AUDIT.json; never implements" ;;
esac; }
...
kickoff() {  # the initial prompt handed to `claude` in the role's terminal
  local role="$1"
  printf 'You are the RAPID %s. Read 04-spec/agents/%s.md (your role contract), .rapid/STATE.json and .rapid/TASKS.json. Call claude-peers set_summary with your role + current focus, list_peers to find the team, then begin: %s. Coordinate over claude-peers and answer teammates immediately.' \
...
tasks.append({"label":"RAPID: launch team","dependsOrder":"parallel",
              "dependsOn":[f"RAPID: {r}" for r in roles],"problemMatcher":[],
...
PY
    echo "In Cursor: Cmd/Ctrl+Shift+P → 'Run Task' → 'RAPID: launch team' — each role opens in its own integrated terminal."
    ;;
  tmux)
    command -v tmux >/dev/null 2>&1 || { echo "rapid-team: tmux not found (brew install tmux) — or use: rapid-team.sh --cursor" >&2; exit 2; }
    S=rapid-team
    if tmux has-session -t "$S" 2>/dev/null; then echo "rapid-team: session '$S' already running — attach: tmux attach -t $S"; exit 0; fi
    first=1
    for r in $ROLES; do
      ensure_brief "$r"

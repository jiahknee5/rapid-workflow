...
def topnav(active):
    parts = []
    for p in PH:
        cls = "step" + (" gate" if p["kind"]=="gate" else "") + (" por" if p.get("por") else "") + (" active" if p["slug"]==active else "")
        label = p["name"] if p["kind"]=="gate" else p["name"]
        tag = "" if p["kind"]=="gate" else f'<i>{p["ph"]}</i> '
        parts.append(f'<a class="{cls}" href="{p["slug"]}.html">{tag}{html.escape(label)}</a>')
    ...
    return ('<nav class="top-nav"><a class="top-brand" href="index.html">RAPID<span class="sub">Skills Atlas</span></a>'
            '<span class="steplabel">Build order →</span>' + inner +
...
<div class="main">
  ...
  <section class="section" id="crosscut">
    ...
    <div class="footnote">Generated by tools/atlas-skills-gen.py · fully Rapid-named · theme docs/atlas-v2-design.md. One page per phase; real names in the header.</div>
  </section>
</div></div></body></html>'''

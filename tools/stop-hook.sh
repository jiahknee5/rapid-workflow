#!/usr/bin/env bash
# RAPID Stop / SubagentStop hook — deterministic halt logging + continuation.
#
...
# the cwd is an active RAPID build (.rapid/STATE.json present). Defensive by
...
#   1. Always append a STOP event to .rapid/observe/<role>.jsonl  (fixes silent halts)
...
# ---- Guard: only act inside an active RAPID build -------------------------
...
case "$PHASE_NUM" in
  ...
  4)
    ...
    ;;
  5)
    testcount=$(find .rapid/EVAL \( -name '*.test.*' -o -name '*.spec.*' -o -name 'tests.ts' -o -name 'tests' \) 2>/dev/null | wc -l | tr -d ' ')
    if [ ! -d ".rapid/EVAL" ] || [ "${testcount:-0}" -lt 1 ] 2>/dev/null; then
      missing=".rapid/EVAL/ tests"
      nextstep="generate the immutable eval harness (task-00) before the build can start"
    elif [ ! -f ".rapid/TASKS.json" ]; then
      missing=".rapid/TASKS.json"
      nextstep="decompose the spec into tasks and write .rapid/TASKS.json"
    fi
    ;;
  6)
...
try:
 t=json.load(open('.rapid/TASKS.json'))
 ...
 print(','.join(str(x.get('id','?')) for x in ts if str(x.get('status','')).lower() not in done))
      ...
      missing=".rapid/P6_EXIT.json"
      if [ -n "$inc" ]; then
        nextstep="continue the build — incomplete tasks: ${inc}. When all tasks pass, run the P6 exit assertions and write .rapid/P6_EXIT.json"
      else
        nextstep="all tasks look done — run the P6 exit assertions (incl. watchdog AUDIT.json check) and write .rapid/P6_EXIT.json"
      fi
    fi
    ;;
  7)
    [ -f ".rapid/WALKTHROUGH.md" ] || {
      missing=".rapid/WALKTHROUGH.md"
      nextstep="run the P7 walkthrough across every surface and write .rapid/WALKTHROUGH.md"; }
    ;;
  *)
...
cnt_file=".rapid/.stop_count"
phase_file=".rapid/.stop_phase"
...
reason=('RAPID phase P${PHASE_NUM} is not complete: required artifact \"${missing}\" does not exist yet. '
        'Do not stop. Next step: ${nextstep}. '
        'If you are genuinely blocked on a human decision or an external dependency, append the blocker to .rapid/MEMORY.md, note it in .rapid/STATE.json, then stopping is allowed.')

#!/usr/bin/env bash
# install-hooks.sh — idempotently wire RAPID's deterministic hooks into the
...
#
# Safe to re-run: it strips any prior RAPID hook entries before re-adding the
# canonical ones (no duplicates), and backs up settings.json first.
set -uo pipefail

TOOLS_DIR="$(cd "$(dirname "$0")" && pwd)"
HOOKS_LINK="$HOME/.claude/hooks/rapid-workflow"
...
B = "bash $HOME/.claude/hooks/rapid-workflow"
...
def strip(ev):
    ...
    for g in hooks.get(ev, []):
        ...
        # groups emptied by stripping were RAPID-only -> dropped
    hooks[ev] = out
...
print("settings: registered Stop, SubagentStop, PreToolUse, PostToolUse (RAPID hooks)")
PY

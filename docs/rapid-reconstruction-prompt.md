...
4. **Phase order is fixed and sequential:** P0 → P1 → P1b → **G0** → P2 → P3 → **G1** → P4 → P5 → P5b → P5c → **G2** → P6 → P7 → P8 → **G3** → P9 → P10. No reordering, no cross-phase parallelism.
5. **Coder fan-out ≤ 4 worktree subagents; the watchdog is separate.** Beyond 4 coder subagents, honest review degrades. Sizing: ≤5 tasks → 1 coder subagent; 6–12 → 2; 13+ → 3–4.
...
| P5 | Task Decomposition + Eval Harness | AUTO | Size coder fan-out; generate immutable `task-00` eval harness; decompose with traceability; generate role files + CI config | `.rapid/TASKS.json`, `.rapid/EVAL/` (locked), `04-spec/agents/*.md`, CI config |
...
| P5c | Mockups & Prototype | AUTO | Hi-fi UI mockups + clickable prototype for every screen; the visual target for P7 | `04-spec/mocks/`, `04-spec/screens.md` |
| **G2** | **Architecture (Point of No Return)** | **HUMAN** | Approve spec/arch/tasks/eval/cost **+ mockups**; collect creds + `.env` (3+3 doc-review subagents) | Spec locked, eval locked, mockups approved, cost budgeted |
...
**Tracks:** `fast` (1 panel area; P6 = planner+coder+reviewer+watchdog, tester folds into coder's keep-or-revert; 3 reviewer dimensions; skip P8 optimization + P10) and `full` (all five panel areas; all 5 leads; up to 9 reviewer dimensions). A `tiny` cadence runs all roles as subagents under a single planner terminal.
...
- **Definition — the sections this file MUST contain, in order:**
  ...
  3. **Deterministic-vs-Dynamic split table** (encode verbatim — DETERMINISTIC: 12 phases in order, 4 gates at fixed positions, 12-message contract + JSON envelope, keep-or-revert, immutable eval harness, cost breaker, shutdown handshake, fixed stop-condition set. DYNAMIC: panels (five areas — Business/Technical/Design/SME/Users — rostered then pared), reviewer weights by domain, debug protocol by stack, optimization vs pillars, 3–5 pillars, spec sections/contracts, coder fan-out 1–4 subagents, test cases from workflow state machine, doc-review criteria from pillars. Dynamic pulls from existing frameworks; never invent protocols at runtime).
  4. **The 12 phases in order**, each with its purpose, the sub-steps from the table above, and the **exit artifact(s)** that R1 enforces. Include: P0 vision + compound-refresh of LEARNINGS; P1 structure + preflight (block until PREFLIGHT green) + Atlas scaffold; P1b decomposition; P2 panels (1 area fast / five areas full, each rostered 1st/2nd/3rd-order then pared, review through pillars, synthesis with convergent/divergent/risks/sources); P3 grounding (VERIFIED/UNVERIFIED/OPEN classification, 15m timebox, 3+ options + trade-off matrix + evidence + fallback + verification); P4 spec derivation (every req tagged `[FROM PRD §X]`/`[DERIVED]`/`[OPEN]`, references a pillar; emit workflow.md + docs/workflows.json wired to same assertions as the harness; CONTRACTS.md; inter-stage assertion every PRD req → spec or `[OUT OF SCOPE]`); P5 task decomposition + immutable eval harness as task-00 + role files + CI config + P5 inter-stage assertions; P5b deepening; P6 five-lead build (full sub-protocol P6a–P6e below); P7 test + visual QA + walkthrough + recorded runs; P8 gap loop + optimization; P9 deploy + document + Atlas deploy; P10 pulse.
  ...
  10. **Panel selection + synthesis** (five areas: Business/Technical/Design/SME/Users, each built 1st/2nd/3rd-order then pared; deepen an area on internal disagreement on load-bearing + irreversible + stakes-threshold).
...
- [ ] `task-00` is the eval-harness root; all tasks depend on it; the planner verifies it `done` before any other; agents extend never edit `.rapid/EVAL/`.

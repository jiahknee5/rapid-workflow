...
- **Decision logging (D5)** — PRD-silent items are LOGGED to `.rapid/DECISIONS.json` via `tools/log-decision.sh "<decision>" spec|interpretation` (basis = PRD-fact vs interpretation), not mid-build interrupts. Only genuinely-undecidable high-stakes items are batched to the human at Gate 1, before the build starts. Wired into the Gate-1 present/collect lists.
- **Preflight (P1 step 6)** — `tools/preflight.sh` probes the declared stack's runtimes, installs missing ones within the declared stack, hard-fails if it can't; writes `.rapid/PREFLIGHT.json`; blocks the build until green. Never code around a missing runtime.
...
- **R1: Phase gate hook** (`rapid-phase-gate.sh`) — blocks STATE.json writes unless required phase artifacts exist. Checks: PRD-ENHANCED.md before P2, synthesis.md before P4, spec.md before P5, EVAL/ with tests before P6, P6_EXIT.json before P7, WALKTHROUGH.md before P8. Prose won't prevent phase-skipping; a hook will.
...
- **Compound learning capture** (P6e step 6) — structured `.rapid/LEARNINGS.md` documenting decisions, patterns, gotchas, and debug traces from each build. Future builds read learnings at P0. Each build makes the next one smarter.
...
- **Claude-peers message contract** — 12 structured JSON message types with durability fallback to `.rapid/MESSAGES.json`.
- **Prompt file generation** — `.rapid/prompts/supervisor.md` and `.rapid/prompts/watchdog.md` generated dynamically at P6a.
...
- **`/rapid-workflow status` dashboard** (Observability) — Reads all .rapid/ files and produces a structured terminal dashboard.
...
### Root cause
Every safeguard was advisory, not enforced. The orchestrator took shortcuts and the skill let it. Fixes add proof files (.rapid/P6_EXIT.json, .rapid/TEST_RESULTS.md) that must exist before phase transitions.

---

## v2.0.0 — 2026-05-26 (Initial rapid skill)
